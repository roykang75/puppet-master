...
test('검색 미리보기: 클릭 → 미리보기(이동 없음) → 더블클릭 → 이동+닫힘', async () => {
  ...
  try {
    ...
    // 구문 강조 실증: colorize가 .mtkN 스팬을 낸다(빈 미리보기가 아니라 강조 렌더).
    await expect(preview.locator('[class*="mtk"]').first()).toBeVisible({ timeout: 10_000 });
    // active 줄이 스크롤로 중앙 정렬되어 뷰포트 안에 보이는지(scrollIntoView 실증)
    await expect(preview.locator('.search-preview-line.active')).toBeVisible();
