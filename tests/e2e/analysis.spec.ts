...
//      보장 불가. util.c 정의 심볼 행(label=helper_fn ∧ detail⊃util.c)을 직접 클릭해
//      "검색 → util.c 점프"를 결정적으로 검증한다 (클릭 픽업도 Enter와 동일 pick() 경로).
...
test('분석 흐름: 검색 점프 → 뒤로 → Context → Relation Callers → 북마크', async () => {
  ...
  try {
    ...
    await page.locator('.search-item', { hasText: 'helper_fn' }).filter({ hasText: 'util.c' }).first().click();
